...
public class BillTableMouseListener extends MouseAdapter 
{

	private final JTable table;

	public BillTableMouseListener(JTable table) 
	{
		this.table = table;
	}
	...
	public void mouseClicked(MouseEvent e) 
	{
		...
		if (row < table.getRowCount() && row >= 0 && column < table.getColumnCount() && column >= 0) 
		{
			...
			if(column==0)
			{
				displayProductDialog(row, column);
...
//				frame.setVisible(true);
	...
	public void displayProductDialog(int row, int column)
	{
		BillingDetailsDialog dia = new BillingDetailsDialog(this.table,row, column);
		JDialog frame = new JDialog(dia.getFrame(), true);
		//DO_NOTHING_ON_CLOSE, HIDE_ON_CLOSE, or DISPOSE_ON_CLOSE
